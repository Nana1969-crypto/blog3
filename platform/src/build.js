...
function loadAll() {
  ...
  // Currículos: fonte única de dados de worldview. Ver docs/protocolo-de-credibilidade.md §1.
  const curFile = path.join(CONTENT, 'curriculum.json');
  const curricula = fs.existsSync(curFile) ? readJson(curFile).curricula : [];
  site.pillars = taxonomy.pillars;
  return { site, authors, taxonomy, articles, pages, certs, curricula };
}
...
function runGate({ articles, authors, taxonomy, certs = [], curricula = [] }) {
  const errors = [];
  ...
  // Rótulo de worldview: nunca inferência, sempre citação rastreável.
  // Regra do protocolo §1 — "sem fonte, sem rótulo" — cobrada aqui, não na boa vontade.
  const WORLDVIEWS = ['christian', 'secular', 'unstated'];
  const AFFILIATIONS = ['religious', 'secular', 'unstated'];
  for (const c of curricula) {
    const at = `curriculum:${c.id}`;
    if (!c.id || !c.name) { err(at, 'entrada sem id/name (CUR-1)'); continue; }
    // Rotular por currículo+matéria, nunca por editora inteira (CUR-2).
    if (!c.subject) err(at, 'sem subject — rótulo por editora inteira é proibido (CUR-2)');
    if (!WORLDVIEWS.includes(c.contentWorldview)) {
      err(at, `contentWorldview inválido: ${c.contentWorldview} (use ${WORLDVIEWS.join('|')}) (CUR-3)`);
    }
    if (!AFFILIATIONS.includes(c.publisherAffiliation)) {
      err(at, `publisherAffiliation inválido: ${c.publisherAffiliation} (use ${AFFILIATIONS.join('|')}) (CUR-3)`);
    }
    // Evidência obrigatória: link + frase textual da própria editora + data (CUR-4).
    if (!c.worldviewSource || !/^https?:\/\//.test(c.worldviewSource)) {
      err(at, 'sem worldviewSource verificável — sem fonte, sem rótulo (CUR-4)');
    }
    if (!c.worldviewQuote || c.worldviewQuote.trim().length < 15) {
      err(at, 'sem worldviewQuote (frase textual da editora, mín. 15 chars) (CUR-4)');
    }
    if (!/^\d{4}-\d{2}$/.test(c.verifiedOn || '')) err(at, 'sem verifiedOn AAAA-MM (CUR-4)');
    // Escopo declarado: o site nunca afirma ter lido o curso inteiro (CUR-5).
    if (!c.scopeChecked || c.scopeChecked.trim().length < 20) {
      err(at, 'sem scopeChecked — o escopo do que foi verificado é obrigatório (CUR-5)');
    }
  }
  return errors;
}
...
function build(opts = {}) {
  const t0 = Date.now();
  const { site, authors, taxonomy, articles, pages, certs, curricula } = loadAll();

  // GATE
  const gateErrors = runGate({ articles, authors, taxonomy, certs, curricula });
...
`);
  ...
  // Mesma vigilância de calendário para os rótulos de worldview: editora revisa
  // material, rótulo envelhece. O motor cobra a revisão, nunca adivinha o valor.
  const staleCur = staleCerts(curricula);
  if (staleCur.length) {
    console.warn(`\n⚠  REVISAR RÓTULOS — ${staleCur.length} currículo(s) sem verificação há ${STALE_AFTER_MONTHS}+ meses:`);
    for (const c of staleCur) console.warn(`   ${c.name}: verificado em ${c.verifiedOn} (${c.months} meses atrás)`);
    console.warn('   Reabra a página da editora, reconfira a citação e atualize content/curriculum.json (incluindo verifiedOn).\n');
  }
}
